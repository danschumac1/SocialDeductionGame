...
introduce_yourself_prompt = PromptTemplate(
    input_variables=["code_name", "team"],
    template="Introduce yourself. You are code name {code_name}, and you are aligned with the {team} team."
)

# CHOSE ACTION PROMPT
actionOption_parser = PydanticOutputParser(pydantic_object=ActionOptionBM)
chose_action_prompt = PromptTemplate(
    template="""Given the current game context, select ONLY ONE action and respond accordingly.
                You must return a JSON object with exactly one of these keys filled:
                
                {format_instructions}
                Ensure the response follows this rule strictly.""",
    input_variables=[],
    partial_variables={"format_instructions": actionOption_parser.get_format_instructions()}
)

# GAME SUMMARY PROMPT
gameSummary_parser = PydanticOutputParser(pydantic_object=GameSummaryBM)
game_summary_prompt = PromptTemplate(
    input_variables=["previous_summary", "minutes"],
    template="""Generate a summary of the game so far. Include the round number, the players that
        ...
        last vote outcome, and textual summary of the game so far.
        PREVIOUS SUMMARY: {previous_summary}.
        MINUTES: {minutes}
    
        {format_instructions}
        Ensure the response is a JSON object and follows the structure exactly.""",
    partial_variables={"format_instructions": gameSummary_parser.get_format_instructions()}
        )

# DEFEND YOURSELF PROMPT
defend_yourself_parser = PydanticOutputParser(pydantic_object=DefendYourselfBM)
defend_yourself_prompt = PromptTemplate(
    input_variables=["accuser", "accusation", "current_dialogue"],
    template="""You are playing a social deduction game where humans are trying to identify robots.
    
    You have just been accused by {accuser}. Here is their accusation:
    "{accusation}"
    ...
    Ensure the response is a JSON object and follows the structure exactly.
    """,
    partial_variables={"format_instructions": defend_yourself_parser.get_format_instructions()}
)

# ACCUSE PLAYER PROMPT
accuse_player_parser = PydanticOutputParser(pydantic_object=AccusePlayerBM)
accuse_player_prompt = PromptTemplate(
    input_variables=["current_dialogue", "game_state_summary"],
    template = """You are playing a social deduction game where humans are trying to identify robots.
        ...
        PREVIOUS VOTES: {previous_votes}  # ✅ Added to justify accusations
        ...
        Ensure the response is a JSON object and follows the structure exactly.""",
    partial_variables={"format_instructions": accuse_player_parser.get_format_instructions()}
)
...
joke_prompt = PromptTemplate(
    ...
    template="""You are playing a social deduction game, and you want to tell a joke to make yourself seem more human.
...
question_prompt = PromptTemplate(
    input_variables=["game_state", "current_dialogue"],
    template="""You are playing a social deduction game where humans are trying to find robots.
...
simple_phrase_prompt = PromptTemplate(
    input_variables=["conversation_context"],
    template="""You are playing a social deduction game and want to respond with a simple phrase.
